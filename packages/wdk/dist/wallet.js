...
const DEFAULT_MAX_FEE = 1000000000000000n;
...
export class AgentWallet {
    ...
    constructor(seedPhrase, config) {
        if (!seedPhrase || seedPhrase.trim().split(" ").length < 12) {
            throw new Error("Invalid seed phrase: must be a BIP-39 mnemonic (12–24 words).");
        }
        if (!config.rpcUrl) {
...
export function createAgentWallet() {
    ...
    if (!seed) {
        throw new Error("Missing env var AGENT_SEED_PHRASE. Set it in your .env file.");
    }
    if (!rpcUrl) {
